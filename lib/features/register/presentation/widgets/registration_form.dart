      ...
      if(_formKey.currentState?.validate() ?? false)
        {
          final newUser = RegistrationModel(
           ...
           bool registrationStatus = await ref.read(registerProvider.notifier).newUserRegistration(newUser);
           if(registrationStatus)
             {
                 ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Registration Successfully Done"), backgroundColor: Colors.green, duration: Duration(seconds: 4),));
                 context.go('/home');
             }
           else
             {
               ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Registration Failed"), backgroundColor: Colors.red, duration: Duration(seconds: 4),));
             }
        }
  }
  ...
  Widget build(BuildContext context) {
    return Form(
        ...
        child: Column(
          ...
          ElevatedButton(onPressed: _handleRegistration, child: Text("Sign Up")),
            const SizedBox(height: 20),
          Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              Text("Already have an account?"),
              TextButton(onPressed: () => context.pushReplacement('/login'), child: Text("Sign In")),
            ],
          )
        ],));
  }
